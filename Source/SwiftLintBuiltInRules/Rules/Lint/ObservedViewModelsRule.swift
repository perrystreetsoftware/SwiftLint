...
struct ObservedViewModelsRule: SwiftSyntaxRule, ConfigurationProviderRule {
...
private extension ObservedViewModelsRule {
    final class Visitor: ViolationsSyntaxVisitor {
        ...
        override func visit(_ node: MemberDeclListItemSyntax) -> SyntaxVisitorContinueKind {
            ...
            if let decl = node.as(MemberDeclListItemSyntax.self)?.decl.as(VariableDeclSyntax.self) {
                if decl.attributes?.first?.as(AttributeSyntax.self)?.attributeName.as(SimpleTypeIdentifierSyntax.self)?.name.text == Self.ObservedObjectKeyword {
                    observed = true
                }
        ...
        override func visitPost(_ node: MemberAccessExprSyntax) {
            if let node = node.as(MemberAccessExprSyntax.self) {
                if let identifierText = node.base?.as(IdentifierExprSyntax.self)?.identifier.text {
                    if observedViewModels.contains(identifierText.replacingOccurrences(of: "$", with: "")) &&
                        node.dot.tokenKind == TokenKind.period {
