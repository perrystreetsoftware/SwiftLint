...
private extension AdaptersUseRefsNotClosuresRule {
    final class Visitor: ViolationsSyntaxVisitor<ConfigurationType> {
        ...
        func isDeclaredInSwiftUiBody(_ node: Syntax?) -> Bool {
            ...
            let isReturningView = patternListBinding.first?.typeAnnotation?.as(TypeAnnotationSyntax.self)?.type.as(SomeOrAnyTypeSyntax.self)?.constraint.as(IdentifierTypeSyntax.self)?.name.text == "View"
